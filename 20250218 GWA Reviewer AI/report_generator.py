...
def create_wordcloud(text_descriptions):
    ...
    custom_stopwords.update(['image', 'Image', 'camera', 'Camera', 'b', "summary", "Summary"])
...
class ReportTemplate(BaseDocTemplate):
    ...
    def on_later_pages(self, canvas, doc):
        # Add logo to footer
        logo_path = BASE_DIR/ "source/report" / "20250214 GWA Logo LONG WHITE LIGHT.png"
        if logo_path.exists():
...
def create_report(postfix):
    # Setup document
    output_file = BASE_DIR / "OUTPUT" / f"Report_{datetime.now().strftime('%Y%m%d')}{postfix}.pdf"
    doc = ReportTemplate(
        str(output_file),
        pagesize=A4,
        topMargin=inch,
    ...
    # Cover page
    cover_image_path = BASE_DIR / "source" / "report_cover" / "20250217 GWA Reviewer Report Cover.png"
    ...
    story.append(Paragraph(read_text_file(BASE_DIR / "source" / f"report{postfix}" / "report preamble.txt"), normal_style))
    story.append(Spacer(1, 20))
    story.append(Paragraph("Executive Summary", heading_style))
    story.append(Paragraph(read_text_file(BASE_DIR / "source" / f"report{postfix}" / "executive summary.txt"), normal_style))
    story.append(Paragraph("Aggregated Summary", heading_style))
    story.append(Paragraph(read_text_file(BASE_DIR / "source" / f"report{postfix}" / "aggregated summary.txt"), normal_style))
    story.append(PageBreak())
    ...
    for screenshot in screenshots:
        img_path = BASE_DIR / "source" / f"ims{postfix}" / f"{screenshot}.png"
        txt_path = BASE_DIR / "source" / f"ims{postfix}" / f"{screenshot}.txt"
    ...
    if text_descriptions:
        ...
        wordcloud_img.drawHeight = 3.5*inch  # Reduced from 4*inch
        wordcloud_img.drawWidth = 5.9*inch   # Reduced from 7*inch
        story.append(wordcloud_img)
        
        # Read insights from file
        story.append(Spacer(1, 20))
        story.append(Paragraph("Key Insights:", heading_style))
        insights = read_text_file(BASE_DIR / "source" / f"report{postfix}" / "report insights.txt")
        story.append(Paragraph(insights, normal_style))
        #story.append(PageBreak())

    # Final page
    story.append(Paragraph("Conclusion", heading_style))
    story.append(Paragraph(read_text_file(BASE_DIR / "source" / f"report{postfix}" / "report conclusion.txt"), normal_style))
...
if __name__ == "__main__":
    ...
    def show_selection_dialog():
        root = tk.Tk()
        root.title("Report Type Selection")
        ...
        construction_btn = tk.Button(button_frame, text="Construction", command=construction_clicked, width=12)
        construction_btn.pack(side=tk.LEFT, padx=5)
